...
const webpackImportRegex = /import \{(.+?)\} from ['`"]@webpack(\/.+?)?['`"]/;

export const WebpackCodeLensProvider: CodeLensProvider = {
    provideCodeLenses(document) {
        const text = document.getText();

        const match = webpackImportRegex.exec(text);
        if (!match) return [];
        ...
        function walk(node: Node) {
            ...
            if (isCallExpression(node) && finds.includes((type = node.expression.getText()))) {
                const args = node.arguments.map(
                    (a) =>
                        tryParseStringLiteral(a) ?? tryParseRegularExpressionLiteral(a) ?? tryParseFunction(document, a)
                );
                ...
                lenses.push(
                    new CodeLens(range, {
                        title: "Test Find",
                        command: "extendify-companion.testFind",
